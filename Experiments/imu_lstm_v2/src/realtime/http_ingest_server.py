...
from Experiments.imu_lstm_v2.src.realtime.motion_control import ensure_control_state, update_control

app = FastAPI()
YAW_IDX = {"gx": 0, "gy": 1, "gz": 2}

state = {
    "scaler": None, "le": None, "cfg": None, "model": None, "device": None,
    "eff_window": None, "artifacts": None, "threshold": 0.6,
    "per_class_threshold": {}, "debounce_k": 2, "min_acc_for_jump": 12.0,
    "debounce_map": {"default": 3, "straight": 2},
    "threshold_map": {"default": 0.6, "straight": 0.55, "left": 0.7, "right": 0.7},
    "quiet_gyro": 0.06, "quiet_ms": 250, "last_quiet": {},
    "turn_fast_thr": 0.90, "turn_min_gyro": 0.10,
    "ema_probs": {}, "ema_alpha": 0.6,
    "yaw_assist": True, "yaw_axis": "gx", "yaw_sign": 1,
    "yaw_min": 0.12, "yaw_window": 12, "yaw_prior_k": 2.0,
    "yaw_prior_min": 0.08, "yaw_prior_max": 0.35,
    "probs_s": {}, "jump_hold_ms": 140, "jump_lock_until": {},
    "ctl": {}, "max_speed": 1.0, "speed_alpha": 0.25, "yaw_alpha": 0.30,
    "max_yaw_rate_dps": 180.0, "turn_mix": 0.6, "acc_to_speed_gain": 0.08,
    "acc_bias_alpha": 0.01, "speed_floor": 0.0,
    "auto_calib": 0, "auto_calib_done": {},
    # anti-flicker for turns
    "turn_guard_ms": 150.0,
    "last_turn": {},
    "windows": {}, "latest": {}, "recent": {}, "rot": {}, "g_ref": None,
    "hist": {}, "current_cmd": {}, "classes": [], "log_rows": [], "log_dir": None,
}
...
def rot_axis_angle(u, theta):
    u = np.asarray(u, dtype=float); u = u / (np.linalg.norm(u) + 1e-12)
...
def init_model(artifacts: Path, threshold: float, window_override: int = 0):
    ...
    log_dir = Path(artifacts) / "logs"; log_dir.mkdir(parents=True, exist_ok=True)
...
def ensure_window(device_id: str):
    ...
    if device_id not in state["rot"]:
        R = load_rot(device_id); state["rot"][device_id] = (R if R is not None else np.eye(3))
...
def _apply_yaw_prior(device_id: str, probs: np.ndarray, arr_raw: np.ndarray) -> np.ndarray:
    ...
    if yaw > 0: logits[ri] += k * mag
    elif yaw < 0: logits[li] += k * mag
...
def _debounced_command(device_id: str, label: str, confidence: float, now_ts: float, arr_raw: np.ndarray) -> str:
    ...
    classes = list(state["le"].classes_)
    try: j_idx = classes.index("jump")
    except ValueError: j_idx = None

    lock_until = state["jump_lock_until"].get(device_id, 0.0)
    if now_ts < float(lock_until):
        state["current_cmd"][device_id] = "JUMP"; return "JUMP"
    ...
    if j_idx is not None:
        ...
        acc = arr_raw[:, 3:6]; g_hat = normalize(state["g_ref"])
        ...
        if (p_jump >= float(state["turn_fast_thr"])) or (p_jump >= th_jump and a_p90 >= float(state["min_acc_for_jump"])):
            ...
            state["jump_lock_until"][device_id] = now_ts + float(state["jump_hold_ms"]) / 1000.0
            return "JUMP"
    ...
    if label.lower() != "straight":
        if confidence >= float(state["turn_fast_thr"]) and g_mag >= float(state["turn_min_gyro"]):
            ...
            if last and last.get("dir") == opp and (now_ts - float(last.get("ts", 0.0))) * 1000.0 < guard_ms:
                pass
            else:
                state["current_cmd"][device_id] = label.upper()
                state["hist"][device_id].clear()
                state.setdefault("last_turn", {})[device_id] = {"dir": label.upper(), "ts": now_ts}
                return label.upper()
            

    k_map = state["debounce_map"]; k_def = int(k_map.get("default", state["debounce_k"]))
    k = int(k_map.get(label.lower(), k_def))
    hist = state["hist"][device_id]; hist.append(label)
...
def _append_log_row(device_id: str, ts: float, label: str, command: str, confidence: float, probs: Dict[str, float], raw_latest: Dict[str, float]):
    row = {"timestamp": ts, "device_id": device_id, "label": label, "command": command, "confidence": confidence}
    # NEW: log prob_sum to verify normalization
    row["prob_sum"] = float(sum(probs.values()))
    for c in state["le"].classes_: row[f"prob_{c}"] = float(probs.get(c, 0.0))
    if raw_latest:
...
def write_logs_csv(out_dir: str | None = None) -> str | None:
    ...
    out_base = Path(out_dir or state.get("log_dir") or "."); out_base.mkdir(parents=True, exist_ok=True)
...
def _on_exit_flush_logs():
    try:
        p = write_logs_csv()
        if p: print(f"[INFO] Wrote prediction log to {p}")
...
def ingest(payload: Dict[str, Any] = Body(...)):
    try:
        ...
        pred = None
        for s in samples:
            parts = [float(s["gx"]), float(s["gy"]), float(s["gz"]), float(s["ax"]), float(s["ay"]), float(s["az"])]
            parts = apply_map(parts); parts = apply_rotation(parts, device_id)
            ts = int(s.get("ts", time.time() * 1000))
            latest_sample = {"ts": ts, "gx": parts[0],"gy": parts[1],"gz": parts[2],"ax": parts[3],"ay": parts[4],"az": parts[5]}
            state["recent"][device_id].append(latest_sample)
            win.push(parts)

            # one-time auto calibration (optional)
            if state.get("auto_calib", 0) and not state["auto_calib_done"].get(device_id, False) and win.ready():
                arr = win.array(); g_live = arr[:, 3:6].mean(axis=0); R = rot_from_vectors(g_live, state["g_ref"])
                state["rot"][device_id] = R; save_rot(device_id, R); state["auto_calib_done"][device_id] = True

            if not win.ready(): continue
            arr_raw = win.array()
            arr = arr_raw[None, ...]
            arr_s = state["scaler"].transform(arr.reshape(-1, arr.shape[-1])).reshape(arr.shape)
            xb = torch.tensor(arr_s, dtype=torch.float32).to(state["device"])
            with torch.no_grad():
                logits = state["model"](xb); probs = torch.softmax(logits, dim=1).cpu().numpy()[0]

            now_ts = time.time()
            label_g, conf_g, probs_g, _ = _gate_jump_and_decide(probs.copy(), arr_raw)
            probs_p = _apply_yaw_prior(device_id, probs_g, arr_raw) if state.get("yaw_assist", True) else probs_g
            ...
            idx = int(np.argmax(probs_s)); classes = list(state["le"].classes_)
            label = classes[idx]; conf = float(probs_s[idx])
            ...
            pred = {
                "label": label, "command": cmd or "NONE", "confidence": conf,
                ...
                "timestamp": now_ts, "window": state["eff_window"], "control": ctrl
            }
...
def calibrate_gravity(device_id: str = "imu01"):
    ensure_window(device_id); win = state["windows"][device_id]
    if not win.ready(): return {"ok": False, "error": "window not ready"}
    arr = win.array(); g_live = arr[:, 3:6].mean(axis=0); g_ref = state["g_ref"]
    R = rot_from_vectors(g_live, g_ref); state["rot"][device_id] = R; save_rot(device_id, R)
...
def stats(device_id: str = "imu01", scaled: int = 0):
    ...
    if not win or not win.ready(): return {"ok": False, "error": "window not ready"}
    arr = win.array()
    if scaled: arr = state["scaler"].transform(arr)
    st = window_stats(arr)
    return {"ok": True, "scaled": bool(scaled), "mean": st["mean"], "std": st["std"], "labels": ["gx","gy","gz","ax","ay","az"]}

@app.get("/latest")
def latest(device_id: str = "imu01"):
    last = state["latest"].get(device_id, {"label":"NONE","confidence":0.0,"command":"NONE"})
    ctl = state.get("ctl", {}).get(device_id)
    if ctl and isinstance(last, dict) and "control" not in last:
        last = dict(last); last["control"] = {
...
@app.get("/config")
def get_config():
    return {"threshold": state["threshold"], "effective_window": state["eff_window"],
...
def swap_left_right(device_id: str = "imu01"):
    ensure_window(device_id); R = state["rot"].get(device_id, np.eye(3))
    Rpi = rot_axis_angle(state["g_ref"], np.pi); R_new = Rpi @ R
    state["rot"][device_id] = R_new; save_rot(device_id, R_new)
...
def flush_log(out_dir: str | None = None):
    ...
    if p is None: return {"ok": True, "written": 0, "path": None}
    n = len(state.get("log_rows", [])); state["log_rows"].clear()
    return {"ok": True, "written": n, "path": p}

@app.get("/control")
def get_control(device_id: str = "imu01"):
    ensure_window(device_id); ctl = state.get("ctl", {}).get(device_id)
    if not ctl: return {"ok": False, "error": "no control yet"}
...
def reset_angle(device_id: str = "imu01"):
    ensure_window(device_id); ctl = state.get("ctl", {}).get(device_id)
    if ctl: ctl["angle_deg"] = 0.0
    return {"ok": True, "device_id": device_id, "angle_deg": float(ctl.get("angle_deg", 0.0) if ctl else 0.0)}

@app.post("/set_yaw")
def set_yaw(device_id: str = "imu01", axis: str = "gx", sign: int = 1):
    axis = axis.lower()
    if axis not in ("gx","gy","gz"): return {"ok": False, "error": "axis must be gx|gy|gz"}
    state["yaw_axis"] = axis; state["yaw_sign"] = int(1 if sign >= 0 else -1)
...
def auto_yaw(device_id: str = "imu01"):
    win = state["windows"].get(device_id)
    if not win or not win.ready(): return {"ok": False, "error": "window not ready"}
    arr = win.array(); stds = arr[:, 0:3].std(axis=0); axes = ["gx","gy","gz"]
    axis = axes[int(np.argmax(stds))]; state["yaw_axis"] = axis
    return {"ok": True, "axis": axis, "sign": state["yaw_sign"], "std": {axes[i]: float(stds[i]) for i in range(3)}}
...
if __name__ == "__main__":
    ...
    state["threshold_map"]["straight"] = (float(args.threshold_straight) if args.threshold_straight > 0 else max(0.0, float(args.threshold) - 0.05))
    ...
    state["threshold_map"]["jump"] = (args.threshold_jump if args.threshold_jump > 0 else float(args.threshold))
    state["quiet_gyro"] = float(args.quiet_gyro); state["quiet_ms"] = int(args.quiet_ms)
    state["turn_fast_thr"] = float(args.turn_fast_thr); state["turn_min_gyro"] = float(args.turn_min_gyro)
    state["ema_alpha"] = float(args.ema_alpha); state["yaw_prior_k"] = float(args.yaw_prior_k)
    state["yaw_prior_min"] = float(args.yaw_prior_min); state["yaw_prior_max"] = float(args.yaw_prior_max)
    state["jump_hold_ms"] = int(args.jump_hold_ms)
    state["max_speed"] = float(args.max_speed); state["speed_alpha"] = float(args.speed_alpha); state["yaw_alpha"] = float(args.yaw_alpha)
    state["max_yaw_rate_dps"] = float(args.max_yaw_rate_dps); state["turn_mix"] = float(args.turn_mix)
    state["acc_to_speed_gain"] = float(args.acc_to_speed_gain); state["acc_bias_alpha"] = float(args.acc_bias_alpha); state["speed_floor"] = float(args.speed_floor)
